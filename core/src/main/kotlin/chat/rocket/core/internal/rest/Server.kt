...
suspend fun RocketChatClient.serverInfo(): ServerInfo = withContext(Dispatchers.Default) {
...
suspend fun RocketChatClient.configurations(): Map<String, Map<String, String>> = withContext(Dispatchers.Default) {
 ...
 * @since 0.63.0
 */
suspend fun RocketChatClient.settingsOauth(): SettingsOauth = withContext(Dispatchers.Default) {
    val url = restUrl.newBuilder()
        .addPathSegment("api")
        .addPathSegment("v1")
...
suspend fun RocketChatClient.settings(vararg filter: String): Map<String, Value<Any>> = withContext(Dispatchers.Default) {
    val url = restUrl.newBuilder().apply {
        addPathSegment("api")
        addPathSegment("v1")
